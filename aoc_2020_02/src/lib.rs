...
pub fn aoc_2020_02_a(input: &str) -> usize {
    let re: Regex = Regex::new(r"^\s*(?P<min>\d)\s*-\s*(?P<max>\d)\s+(?P<letter>\w): (?P<pwd>\w+)\s*$").unwrap();

    // 1-3 a: abcde
    input.trim().lines().flat_map(|s| re.captures(s)).flat_map(|&caps| {
        ...
        if letter_count < min || letter_count > max {0} else {1}
    }).sum()
}
